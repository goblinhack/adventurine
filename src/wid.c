    ...
    for (z = 0; z < MAP_DEPTH; z++) {
        for (i = 0; i < grid->nelems; i++) {
            TREE_WALK(grid->grid_of_trees[z][i], node) {
                ...
                count = (uint32_t) (uintptr_t) wid_get_context(child);
...
uint8_t demarshal_wid_grid (levelp level,
                            demarshal_p ctx, widp w,
                                    ...
                                    const int light_index)
{
    ...
    light_radius *= game.tile_width;
    light->strength *= game.tile_height;
                                 ...
                                 const int light_level)
{
    ...
    double visible_width = light_radius + 5;
    double visible_height = light_radius + 5;
