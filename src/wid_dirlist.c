...
static void wid_dirlist_destroy (widp w)
{
    ...
    context = (typeof(context))
                    wid_get_context(wid_get_top_parent(w));
                  ...
                  float x, float y, int32_t args, ...)
{
    ...
    if (context->maxw > (uint32_t) ((game.video_gl_width * 3) / 4)) {
        need_horiz_scroll = true;
    }

    context->orig_maxw = context->maxw;
    context->maxw =
        min(context->maxw, (uint32_t) ((game.video_gl_width * 3) / 4));
...

    {
        ...
        wid_set_context(wid_dirlist_window, context);
