...
static void demarshal_thing_tile (demarshal_p ctx, thing_tile *t)
{
    ...
    do {
        ...
        GET_OPT_NAMED_BITFIELD(ctx, "is_dir_none", none);
...
static void marshal_thing_tile (marshal_p ctx, thing_tile *t)
{
    ...
    switch (t->dir) {
    case THING_DIR_NONE:
        PUT_NAMED_BITFIELD(ctx, "is_dir_none", 1);
        break;
    case THING_DIR_LEFT:
...
void demarshal_thing_tiles (demarshal_p ctx, tpp t)
{
    ...
    while (GET_PEEK_NAME(ctx, "tile")) {
        ...
        tile = &thing_tile_arr[thing_tile_count];
        newptr(tile, "tile");

        tile->tree.key = id++;
        tile->array_index = thing_tile_count++;

        if (!tree_insert_static(t->tiles, &tile->tree.node)) {
