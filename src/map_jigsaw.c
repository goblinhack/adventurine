...
#define MAZE_MIN_DISTANCE_START_AND_END                 40
#define MAX_MAP_NUMBER_OF_TIMES_TO_TRY_AND_PLACE_FRAG   5
#define MAZE_ALLOW_NON_EXACT_MATCH_BETWEEN_CELLS
...
static void maze_debug(dungeon_t *, frag_t *);
#endif
...
void map_jigsaw_fini (void)
{
    ...
    if (frag_room_corners) {
        myfree(frag_room_corners);
    }

    if (frag_outside_rooms) {
        myfree(frag_outside_rooms);
    }

    if (frag_outside_junk) {
        myfree(frag_outside_junk);
    }
#endif
 ...
 */
static void jigpieces_read (frag_t *fragmap, char *buf)
{
    char *c = buf;
...
static int32_t jigpiece_char_is_suitable_for_exit (char c)
{
    return (c == MAP_FLOOR)             ||
           (c == MAP_CORRIDOR)          ||
           (c == MAP_EMPTY);
 ...
 */
static void jigpiece_create_exits (frag_t *fragmap)
{
    int32_t which;
    int32_t x;
    int32_t y;
    ...
    for (which = 0; which < fragmap->jigpieces_cnt; which++) {
        ...
        printf("made exits for frag %d\n", which);
        jigpiece_print(fragmap, which);
#endif
    }
}
...
static void jigpiece_get_bounds (jigpiece_t *j)
{
    if (j->set_min_max) {
        return;
    }
 ...
 */
static void jigpiece_create_mirrored_pieces (frag_t *fragmap)
{
    int32_t c;
    int32_t x;
    int32_t y;
 ...
 */
static void jigpiece_create_mirrored_frag (frag_t *fragmap)
{
    int32_t c;
    int32_t x;
    int32_t y;
 ...
 */
static void jigpiece_create_mirrored_frag_alt (frag_t *fragmap)
{
    int32_t c;
    int32_t x;
    int32_t y;
...
{
#ifdef MAZE_ALLOW_NON_EXACT_MATCH_BETWEEN_CELLS
    uint16_t A = fragmap->jigpiece[a].exits[dir];
    uint16_t B = fragmap->jigpiece[b].exits[3 - dir];

    /*
     * As long as there is some exit between the two, allow them
     */
    if ((A & B)) {
        return (1);
    }
 ...
 */
#if 0
static void maze_add_borders_around_rooms_and_corridors (void)
{
    int32_t x;
    int32_t y;
    int32_t dx;
 ...
 * Wrap all corridors in walls
 */
static void maze_add_final_borders (void)
{
    int x, y;
...
maze_replace_room_char (char new_char)
{
    ...
    while (tries--) {
        ...
        /*
         * Entrance at the top, exit at the bottom.
         */
        if (new_char == MAP_START) {
            cy = myrand() % (MAP_HEIGHT / MAP_JIGSAW_PIECES_DOWN);
            cy += MAP_BORDER;
        }

        if (new_char == MAP_END) {
            cy = MAP_HEIGHT - (myrand() % (MAP_HEIGHT / MAP_JIGSAW_PIECES_DOWN));
            cy -= MAP_BORDER;
            cy --;
        }
...
static void dump_jigpieces_to_map (dungeon_t *dg, frag_t *fragmap)
{
    ...
    for (x = 0; x < MAP_WIDTH; x++) {
        ...
        for (y = MAP_HEIGHT - dy; y <= MAP_HEIGHT; y++) {
            map_jigsaw_buffer_goto(x, y);
            map_jigsaw_buffer_putchar(MAP_WALL);
        }
    }
    ...
    for (y = 0; y < MAP_HEIGHT; y++) {
        ...
        for (x = MAP_WIDTH - dx; x <= MAP_WIDTH; x++) {
            map_jigsaw_buffer_goto(x, y);
            map_jigsaw_buffer_putchar(MAP_WALL);
        }
    }
 ...
 * maze_debug
 */
static void maze_debug (dungeon_t *dg, frag_t *fragmap)
{
    system("clear");
        ...
        for (x = 0; x < w; x++) {
            ...
            for (c = 1; c < fragmap->jigpieces_cnt; c++) {
                ...
                for (dir = 0; dir < DIR_MAX; dir++) {
                    ...
                    if (b && !a) {
#ifdef MAZE_DEBUG_PRINT_EXITS
    printf("skip frag: frag %d does not have exit %d (%d v %d) has %02x %02x %02x %02x\n", 
        c, dir,
        a, b, 
    fragmap->jigpiece[c].exits[DIR_NORTH],
    fragmap->jigpiece[c].exits[DIR_EAST],
    fragmap->jigpiece[c].exits[DIR_WEST],
    fragmap->jigpiece[c].exits[DIR_SOUTH]);
    jigpiece_print(fragmap, c);
#endif
                        break;
                    }
                }
            ...
            if (!mcell->possible_jigpieces_size) {
                maze_print_cells(dg);
                DIE("cell %d %d has no possible jigpiece\n",x,y);
                             ...
                             uint32_t *count)
{
    ...
    for (p = 0; p < mcell->possible_jigpieces_size; p++) {
        ...
        for (dir = 0; dir < DIR_MAX; dir++) {
             ...
             */
            if (!jigpiece_intersect_score(fragmap, c, dir, 
                                          ocell->jigpiece_index)) {
                break;
            }
        }
    ...
    for (i=0; i < 1000; i++) {
        ...
        if (jp->has[MAP_SHOP_FLOOR]) {
            for (m = 0; m < MAP_JIGSAW_PIECES_ACROSS * MAP_JIGSAW_PIECES_DOWN; 
                    m++) {
 ...
 * generate_level
 */
static int32_t generate_level (dungeon_t *dg,
                               frag_t *fragmap)
{
    for (;;) {
 ...
 * finalize_level
 */
static int32_t finalize_level (dungeon_t *dg)
{
...
static void init (void)
{
    ...
    map_fg[MAP_WALL_RANDOM]    = TERM_COLOR_WHITE;
    map_fg[MAP_OBSTACLE]       = TERM_COLOR_WHITE;
    map_fg[MAP_OBSTACLE_RANDOM]= TERM_COLOR_WHITE;
    map_fg[MAP_SPIKES]         = TERM_COLOR_WHITE;
    map_fg[MAP_SPIKES_RANDOM]  = TERM_COLOR_WHITE;
    ...
    map_bg[MAP_WALL_RANDOM]    = TERM_COLOR_BLUE;
    map_bg[MAP_OBSTACLE]       = TERM_COLOR_BLUE;
    map_bg[MAP_OBSTACLE_RANDOM]= TERM_COLOR_BLUE;
    map_bg[MAP_SPIKES]         = TERM_COLOR_BLUE;
    map_bg[MAP_SPIKES_RANDOM]  = TERM_COLOR_BLUE;
    ...
    valid_frag_char[MAP_WALL_RANDOM]    = true;
    valid_frag_char[MAP_OBSTACLE]       = true;
    valid_frag_char[MAP_OBSTACLE_RANDOM]= true;
    valid_frag_char[MAP_SPIKES]         = true;
    valid_frag_char[MAP_SPIKES_RANDOM]  = true;
    ...
    valid_frag_alt_char[MAP_WALL_RANDOM]    = true;
    valid_frag_alt_char[MAP_OBSTACLE]       = true;
    valid_frag_alt_char[MAP_OBSTACLE_RANDOM]= true;
    valid_frag_alt_char[MAP_SPIKES]         = true;
    valid_frag_alt_char[MAP_SPIKES_RANDOM]  = true;
                           ...
                           tpp *gen)
{
    ...
    case MAP_WALL: 
        ...
        /*
         * Place gold on top of rocks sometimes.
         */
        {
 ...
 * map_jigsaw_generate
 */
void map_jigsaw_generate (levelp level, 
                          int depth,
                          level_replace_thing_t callback)
{
    int got_start = 0;
    int got_end = 0;
    int x, y, z;
    ...
    if (level) {
        maze_seed += level->level_no * 1001 + level->level_no;
    } else {
        maze_seed += depth * 1001 + depth;
    }
    ...
    for (;;) {
        if (++fail_count > 1000) {
            DIE("Cannot seem to make any level");
        }

        memset(dg, 0, sizeof(*dg));

        if (level) {
            dg->depth = 1 + level->level_no;
        } else {
            dg->depth = depth;
        }
        ...
        LOG("Maze: level %u, seed %u", dg->depth, maze_seed);

        if (!generate_level(dg, frag_main)) {                      
            LOG("Maze: generate level %u, failed, seed %u", 
                dg->depth, maze_seed);
            maze_seed += myrand();
            continue;
        }
        ...
        if (!generate_add_frags(dg, frag_room_corners,
                                20 + (dg->depth * 20), // repeat count
                                100         // chance to place fragment
                                )) {
            LOG("Maze: generate level %u, failed, add rooms corners frags, seed %u", 
                dg->depth, maze_seed);
            maze_seed += myrand();
            continue;
        }
        ...
        if (!generate_add_frags(dg, frag_outside_rooms,
                                1,     // repeat count
                                dg->depth  // chance to place fragment
                                )) {
            LOG("Maze: generate level %u, failed, add rooms frags, seed %u", 
                dg->depth, maze_seed);
            maze_seed += myrand();
            continue;
        }
        ...
        if (!generate_add_frags(dg, frag_room_corners,
            ...
            LOG("Maze: generate level %u, failed, add corners frags, seed %u", 
                dg->depth, maze_seed);
            maze_seed += myrand();
            continue;
        }
        ...
        if (!generate_add_frags(dg, frag_outside_junk,
            ...
            LOG("Maze: generate level %u, failed, add junk frags, seed %u", 
                dg->depth, maze_seed);
            maze_seed += myrand();
            continue;
        }
    ...
    for (y = 0; y < MAP_HEIGHT; y++) {
        for (x = 0; x < MAP_WIDTH; x++) {
            ...
            if (map_jigsaw_buffer_water_depth[x][y] == -1) {
                ...
                map_tp[x][y][MAP_DEPTH_FLOOR2] = id_to_tp(THING_WATER1);
            ...
            if (map_jigsaw_buffer_lava_depth[x][y] == -1) {
                ...
                map_tp[x][y][MAP_DEPTH_FLOOR2] = id_to_tp(THING_LAVA1);
            ...
            if (map_jigsaw_buffer_acid_depth[x][y] == -1) {
                ...
                map_tp[x][y][MAP_DEPTH_FLOOR2] = id_to_tp(THING_ACID1);
            ...
            /*
             * Make sure nothing overlaps player
             */
            if (c == MAP_START) {
                got_start = 1;
                continue;
        ...
        for (y = 0; y < MAP_HEIGHT; y++) {
            for (x = 0; x < MAP_WIDTH; x++) {
                for (z = 0; z < MAP_DEPTH; z++) {
                    tpp tp = map_tp[x][y][z];
                    if (tp) {
                        int depth = tp_get_z_depth(tp);
                        tpp_data tp_data = &map_tp_data[x][y][depth];
