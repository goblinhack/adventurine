...
{
    wid_map_ctx *ctx = wid_get_context(w);
    verify(ctx);
...
static void wid_map_find_exits (int x, int y)
{
    ...
    for (mz = 0; mz < MAP_DEPTH; mz++) {
        ...
        if (tp_is_exit(tp) && data->exit_set) {
            ...
            uint32_t level_no = data->exit;

            int e;
            for (e = 0; e < WID_MAP_EXITS_MAX; e++) {
                if (!map->exit_level_no[e]) {
                    map->exit_level_no[e] = level_no;
                    map->exit_at_mx[e] = mx;
                    map->exit_at_my[e] = my;
                    map->exit_count++;
                    break;
...
static void wid_map_draw_exits (void)
{
    ...
    for (y = 1; y < LEVELS_ACROSS; y++) {
        ...
        for (e = 0; e < map->exit_count; e++) {
            ...
            int ex = map->exit_level_no[e] % LEVELS_ACROSS;
            int ey = map->exit_level_no[e] / LEVELS_ACROSS;
...
static void wid_map_preview_do (int thumbnail)
{
    ...
    for (z = 0; z < MAP_DEPTH; z++) {
        ...
        if (!thumbnail) {
            ...
            double offset = 16;
            tl.x += (double) game.video_gl_width / offset;
            tl.y += (double) game.video_gl_height / offset;
            br.x += (double) game.video_gl_width / offset;
            br.y += (double) game.video_gl_height / offset;
        }
...
static void wid_map_preview_small (widp b, fpoint tl, fpoint br)
{
    ...
    int xy = (typeof(xy)) (uintptr_t) wid_get_context2(b);
    int lx = (xy & 0xff);
    int ly = (xy & 0xff00) >> 8;
    ...
    for (z = 0; z < MAP_DEPTH; z++) {
        ...
        tile_blit_fat(0, tile, 0, tl2, br2);
    }
                                                  ...
                                                  tpp_data data)
{
    ...
    int lx = ctx->loading_level_no % LEVELS_ACROSS;
    int ly = ctx->loading_level_no / LEVELS_ACROSS;

    wid_map_level *map = &ctx->levels[ly][lx];
    if (!map) {
        ERR("no map to write to at position (%f,%f) -> (%d,%d) in level %d, "
            "map bounds (%d,%d) -> (%d,%d)", 
            x, y, ix, iy, ctx->loading_level_no,
            0, 0, MAP_DEPTH, MAP_HEIGHT);
    }

    if ((ix >= MAP_WIDTH) || (iy >= MAP_HEIGHT) || (ix < 0) || (iy < 0)) {
        ERR("overflow in reading position (%f,%f) -> (%d,%d) in level %d, "
            "map bounds (%d,%d) -> (%d,%d)", 
            x, y, ix, iy, ctx->loading_level_no,
            0, 0, MAP_DEPTH, MAP_HEIGHT);
        return (0);
...
static void wid_map_load_level (wid_map_ctx *ctx, const char *name, int level_no)
{
    int x = level_no % LEVELS_ACROSS;
    int y = level_no / LEVELS_ACROSS;
     ...
     */
    if (ctx->loading_level_no >= (LEVELS_ACROSS * LEVELS_DOWN)) {
        return;
    }

    if (ctx->loading_level_no < 0) {
        return;
    }
...
static void wid_map_load_levels (wid_map_ctx *ctx)
{
    ...
    { TREE_WALK(d, n) {
        ...
        if (sscanf(name, "%d", &level_no) != 1) {
            ERR("bad format in level name [%s], expecting a,b format", name);
            continue;
        }

        wid_map_load_level(ctx, name, level_no);
    } }
    ...
    while (ramfile->filename) {
        ...
        wid_map_load_level(ctx, ramfile->filename, level_no);
        ramfile++;
    }
}
